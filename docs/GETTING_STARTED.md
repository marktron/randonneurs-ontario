...
| `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | Cloudflare Turnstile site key for rider sign-in (optional; leave unset locally) | [Cloudflare dashboard](https://dash.cloudflare.com/) — production/preview only, see `docs/rider-accounts.md` |
...
Optionally, load membership data and rider PII (email, ccn_id, etc.):

```bash
psql -h localhost -p 54322 -U postgres -d postgres -f supabase/seed-memberships.sql
```

This file is gitignored and generated by `./scripts/generate-seed.sh`. See `docs/database-setup.md` for details.
