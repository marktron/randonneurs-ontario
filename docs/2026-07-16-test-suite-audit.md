...
> **Correction (post-review):** the first draft flagged "chapter_admin can mutate
> another chapter's data" as a **P0** authz vulnerability. That is **wrong** —
> cross-chapter admin access is **intended** (chapter admins help other chapters).
> `docs/SECURITY.md` → "Admin roles" now documents this so it is not re-flagged.
> The finding has been downgraded to N/A throughout.

> **Applied this session (P0 follow-ups):**
>
> - Added `tests/unit/lib/rate-limit.test.ts` — 7 tests for the sliding-window
>   limiter (allow-to-limit, block-past-limit, window expiry, window slide, key
>   isolation, store isolation, reset). Validated: green, and a `>=`→`>` mutation
>   of the limiter fails all 7. **Runs in CI `verify`.**
> - Rewrote `tests/integration-real/token-column-security.test.ts` to seed a
>   registration + result carrying **known** token UUIDs, closing the
>   empty-DB vacuity (E-row below). Typechecks; **not executed here** (needs local
>   Postgres) — run `npm run test:integration-real` to validate before merge.
...
1. **Authorization is universally stubbed at the action layer.** 14+ integration
   files `vi.mock('@/lib/auth/get-admin')` so `requireAdmin()` always resolves. No
   test asserts that an admin action actually _calls_ `requireAdmin()` (i.e. that a
   non-admin / unauthenticated caller is rejected). `requireAdmin()`'s own denial
   is covered by `get-admin.test.ts`, so this is a lower-severity gap than it first
   appears — an action that forgot its auth check would still slip through.
...
1. **Unit-test `lib/rate-limit.ts`** (login/registration/check-in brute-force
   protection has zero tests). **P0.**
...
5. **Add action-layer non-admin denial tests** for a representative set of admin
   actions (does each action actually call `requireAdmin()`?). **P2** — lowered
   because `requireAdmin()`'s own denial is already covered by `get-admin.test.ts`.
...
| -------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ----------- |
| ~~P0~~ **N/A** | ~~chapter_admin can mutate another chapter's data~~ — **by design, not a gap**           | `chapter_admin` intentionally has cross-chapter write access (chapter admins help other chapters). See `docs/SECURITY.md` → "Admin roles". Do **not** add chapter-scoping tests or enforcement.                                        | —                                                               | None — retained only to record that this was reviewed and is intended behaviour.                | —           |
...
| ~~High~~ **FIXED** | `tests/integration-real/token-column-security.test.ts` : 2 anon tests                                                           | Was: pass vacuously on empty DB (no token row seeded → `hasToken=false`); view tests never assert columns absent. **Resolved this session** — now seeds a registration + result with known token UUIDs and asserts `leaked === false` + the view returns the row without the token column. (Needs a real-DB run to validate.) | L26-29, L37-40, L55-73 (old)                                   | Done — reseeded per this row                                                                                     |
...
- Scope: `rate-limit.ts` unit tests (P0); `proxy.ts`/`updateSession` redirect test
  (unauthenticated → login); action-layer non-admin denial tests for a
  representative set of admin actions (P2); ACP email routing test (after the
  `getVpEmail` fix). **No chapter-scoping work** — cross-chapter admin access is
  intended (`docs/SECURITY.md` → "Admin roles").
- Benefit: closes the remaining security-shaped gaps.
- Risk: low; these are additive tests (ACP one pairs with a small source fix).
- Files: ~5 tests (+ the ACP `getVpEmail` fix). **Split: rate-limit PR;
  middleware/authz PR; email-routing PR.**
...
**Mutation-testing candidates (selective, not repo-wide):** `lib/rate-limit.ts`,
`lib/brmTimes.ts`, `lib/memberships/service.ts`, `lib/events/finish-time.ts`, and
`lib/admin/redirect.ts` (once `getSafeRedirectUrl` is extracted). These are
logic-dense rate-limit/date/redirect-safety modules where surviving mutants would
