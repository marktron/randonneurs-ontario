...
- Control list, ordered by `position`: name, route km shown as
  "since previous" and "total" on separate lines (`42.3 km from last` / `207 km total` — collection legs say `this route` instead of `total`, to contrast with the `this event` line; the first control of
...
## 7b. Other riders' check-ins on the card

Organizers have always had the live rider × control grid (§9). Riders get a
narrower slice of the same data on their own card: under each control, who
else on the event has checked in there, and when.

### What the rider sees

- **Collapsed by default.** A control that at least one other sharing rider
  has reached gets a quiet footer line beneath the row — `👥 4 other riders
checked in ▾`. Controls nobody else has reached show nothing (no
  "0 riders" noise on every upcoming row). The count never includes the
  viewer: their own check-in is already the row's right-hand status.
- **Tap to unfold.** The list (name and tap time, earliest first, two
  columns from `sm` up) sits _below_ the control row, not inside it, so
  unfolding it never moves the stamp on a checked-in row. The trigger is a
  real `<button aria-expanded aria-controls>`; the list renders only when
  open, and it starts closed on both server and client, so hydration is
  unaffected.
- **Fresh when opened.** The roster is loaded with the card, then re-fetched
  (`getCheckinRoster(token)`) each time a rider opens a list, so what they
  read is current without the page polling all day. A failed refresh
  (offline, rate-limited) keeps the last list they saw rather than blanking
  it; the count updates when a refresh succeeds.

### Opting out

A **Share my check-ins with other riders** switch sits below the control
list (after the finish banner, before the fine print). It is the rider's
own preference, stored per registration as `registrations.share_checkins`
(`BOOLEAN NOT NULL DEFAULT true`, migration
`20260914120000_add_share_checkins.sql`) and written by
`setCheckinSharing(token, { share })`. The switch flips optimistically and
reverts with an inline message if the server refuses. Unlike check-ins it is
never frozen — a rider can turn sharing off after results are submitted.
...
- **Not reciprocal.** Opting out hides the rider from others; it does not
  hide others from them.
- **Only `registered` registrations count.** A cancelled rider's old
  check-ins drop out of every other card.

### Enforcement

`queryRoster` in `lib/actions/brevet-card.ts` is the single source of the
roster for both the card load and the refresh action. It filters on the
embedded registration and rider in PostgREST
(`registrations.status = registered`, `registrations.share_checkins = true`,
`registrations.riders.hidden = false`, `event_controls.event_id = <event>`)
and `toRosterEntries` re-checks every returned row and drops the viewer's
own, so a row appears only when both layers agree. The mock-based unit
tests cover the code layer; `tests/integration-real/brevet-card/sharing.test.ts`
proves the query filters against a real database. The roster query runs in
parallel with the controls and own-check-ins queries; if it alone fails the
card still renders (with no roster) — social context must never take the
rider's ability to check in down with it.
...
All four sections — Controls, Ride organizer, Check-ins, Pre-Rides — are
wrapped in `CollapsibleSection`
(`components/admin/collapsible-section.tsx`). On a phone each table
"cardifies" into one stacked card per row, so a 400 km route's controls run
thousands of pixels tall and bury the check-in grid; folding a section puts
the next heading one swipe away. A folded heading keeps a summary beside it
("12 controls", the organizer's name, "24 riders") so it still says what is
inside.

Mobile defaults: **Controls** and **Ride organizer** start folded, Check-ins
and Pre-Rides start open. Controls starts _open_ when the event has no saved
controls yet — that is the empty state the admin came to fill, and it is also
what the RWGPS auto-load populates.

Two implementation notes worth keeping:

- The fold is a **mobile affordance only.** From `sm` up the body is always
  rendered and the toggle is `sm:hidden`, so an organizer on a laptop sees
  exactly the page they saw before. The desktop layout is unchanged.
- The fold is **CSS (`hidden sm:block`), not conditional rendering.** A
  collapsed section therefore renders identically on the server and the
  client (no hydration flash of an expanded section on a phone), and the
  inputs inside a folded section stay mounted, so unsaved control edits
  survive a collapse.
...
| Concern                 | Location                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Schema                  | `supabase/migrations/20260703120000_add_digital_brevet_card.sql`; rider preference column in `20260828120000_add_brevet_card_type.sql` (§7a); check-in sharing column in `20260914120000_add_share_checkins.sql` (§7b)                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Rider actions           | `lib/actions/brevet-card.ts` — `getBrevetCardByToken`, `checkInAtControl`, `undoCheckin` (both also decide `isFinalControl` for the finish flow, folded into an existing query); `getCheckinRoster` + `setCheckinSharing` for other riders' check-ins and the sharing switch (§7b)                                                                                                                                                                                                                                                                                                                                                                     |
...
| Admin page              | `app/admin/events/[id]/brevet-card/page.tsx` + `components/admin/event-controls-manager.tsx` + `components/admin/event-checkins-grid.tsx` + `components/admin/checkin-map.tsx` (correction-dialog map) + `components/admin/collapsible-section.tsx` (mobile section folding, §9)                                                                                                                                                                                                                                                                                                                                                                       |
| Email                   | `lib/email/templates.ts` (`digitalCardUrl`), wired in `lib/actions/registration/finalize.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Tests                   | `tests/unit/lib/brevet-card.test.ts`, `tests/unit/lib/brevet-card-actions.test.ts`, `tests/unit/components/brevet-card-view.test.tsx`, `tests/unit/lib/location-help.test.ts`, `tests/integration-real/brevet-card/checkin.test.ts`, `tests/integration-real/brevet-card/undo.test.ts`, `tests/integration-real/brevet-card/sharing.test.ts`, `tests/e2e/brevet-card.spec.ts`, `tests/unit/events/finish-result.test.ts`, `tests/integration-real/brevet-card/finish-result.test.ts`, `tests/unit/events/send-result-reminders.test.ts`, `tests/unit/components/result-submission-form.test.tsx`, `tests/unit/components/collapsible-section.test.tsx` |
