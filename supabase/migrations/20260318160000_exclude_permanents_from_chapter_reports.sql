-- Exclude permanent events from chapter report stats, except:
--   - Top Riders (always includes permanents)
--   - When viewing the Permanent chapter specifically
--
-- The condition used:
--   AND (e.event_type != 'permanent'
--        OR p_chapter_id = (SELECT id FROM chapters WHERE slug = 'permanent'))
-- This excludes permanents unless the Permanent chapter is selected.

-- 2. Event stats — exclude permanents
...
-- 3. Participation stats — exclude permanents
...
-- 4. Top riders — NO CHANGE (permanents always included)

-- 5. Year-over-year summary — exclude permanents
...
-- 6. Non-renewed riders — exclude permanents
